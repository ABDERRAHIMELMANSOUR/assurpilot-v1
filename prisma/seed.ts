...
//
// This seed uses upsert so it's safe to run on every deploy.
// It will create missing data but won't duplicate existing data.
// The build script runs this automatically on Railway.
...
async function main() {
  ...
  await prisma.user.upsert({
    ...
    create: {
      ...
      nom: "Martin", prenom: "François", name: "François Martin",
  ...
  const coach = await prisma.user.upsert({
    ...
    create: {
      ...
      nom: "Dubois", prenom: "Claire", name: "Claire Dubois",
      phoneNumber: "+33 6 00 00 00 02",
      role: "SUPERVISEUR",
      teamId: teamAuto.id,
    },
  });
  // Link coach as team supervisor (safe update)
  await prisma.team.update({
    where: { id: teamAuto.id },
    data: { superviseurId: coach.id },
  }).catch(() => {}); // ignore if constraint already met
  ...
  const agent1 = await prisma.user.upsert({
    ...
    create: {
      ...
      nom: "Laurent", prenom: "Marie", name: "Marie Laurent",
  ...
  const agent2 = await prisma.user.upsert({
    ...
    create: {
      ...
      password: hash("agent123"),
      nom: "Durand", prenom: "Pierre", name: "Pierre Durand",
  ...
  // ── Sample calls (only create if no calls exist yet) ──
  ...
  if (existingCalls === 0) {
    ...
    const calls = [
      ...
      { line: lineSante, agent: agent2, caller: "+33 6 31 42 53 64", missed: true,  dur: 0,   at: daysAgo(0,10,22), res: null, notes: null },
      ...
      { line: lineSante, agent: agent2, caller: "+33 6 39 48 57 66", missed: false, dur: 320, at: daysAgo(2,14,40), res: "RAPPEL_PREVU",   notes: "En déplacement" },
    ];
    ...
    console.log(`  ⏭ Skipped calls — ${existingCalls} already exist`);
  }
...
main()
  .catch((e) => {
    ...
    // Don't exit(1) — let the build continue even if seed partially fails
  })
  .finally(async () => {
